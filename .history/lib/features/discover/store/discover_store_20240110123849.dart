...
abstract class _DiscoverStore with Store {
  ...
  Future<void> fetchMoviesRequested() async {
    isLoading = true;
    final model = ObservableFuture(
        _discoverRepository.fetchMoviesRequested(currentPage: currentPage));
    if (currentPage == 1) {
      discoverMovieModel = model;
    } else {
      final nextPageMovies = model.value?.results ?? [];
      await Future.value(discoverMovieModel).then((value) {
        value.results?.addAll(nextPageMovies);
      });
    }
    isLoading = false;
  }
