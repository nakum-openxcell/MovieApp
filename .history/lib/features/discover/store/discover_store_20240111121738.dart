...
abstract class _DiscoverStore with Store {
  ...
  @observable
  MovieDetailsModel moviesLikeThisModel = MovieDetailsModel();
  ...
  @action
  Future fetchMoviesLikeThis({required String movieId}) async {
    isLoading = true;
    try {
      final data =
          await _discoverRepository.moviesLikeThisRequested(movieId: movieId);
      movieDetailsModel = data;
      isLoading = false;
    } on DioError catch (e) {
      movieDetailsModel =
